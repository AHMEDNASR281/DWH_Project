...
CREATE TABLE Dim_Book (
    ...
    NUMofPages INT CHECK (NUMofPages > 0),
...
CREATE INDEX idx_order_statusvalue ON Dim_Order(StatusValue);
...
SELECT @StartDate = '1/1/1900'  -- Set The start and end date 
...
CREATE TABLE Fact_Sales (
    ...
    FOREIGN KEY (FK_OrderID) REFERENCES Dim_Order(OrderID_BK) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (FK_CustomerID) REFERENCES Dim_Customer(CustomerID_BK) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (FK_BookID) REFERENCES Dim_Book(BookID_BK) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE INDEX idx_fact_sales_price ON Fact_Sales(Price);
